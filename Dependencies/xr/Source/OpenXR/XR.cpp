    ...
    struct System::Session::Impl
    {
        ...
        Anchor CreateAnchor(Pose pose, NativeTrackablePtr trackable, XrTime time)
        {
            ...
            if (trackable != nullptr)
                throw std::runtime_error("Anchors created from hit test results not yet supported for OpenXR.");
        ...
        void UpdateAnchor(Anchor& anchor, XrTime time)
        {
            auto anchorSpace = openXRAnchors[anchor.NativeAnchor];
            ...
            {
                anchor.Pose = { spaceLocation.pose.position.x, spaceLocation.pose.position.y, spaceLocation.pose.position.z,
